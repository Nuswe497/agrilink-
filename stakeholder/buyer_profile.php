    ...
    if (empty($full_name) || empty($email)) {
        ...
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_extension, $allowed_extensions)) {
                $new_filename = 'buyer_' . $user_id . '_' . time() . '.' . $file_extension;
        ...
        if (!$error) {
            ...
            if ($stmt->execute()) {
                ...
                $buyer_info['full_name'] = $full_name;
                $buyer_info['phone'] = $phone;
                $buyer_info['email'] = $email;
                $buyer_info['profile_picture'] = $profile_picture;
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (!password_verify($current_pass, $buyer_info['password_hash'])) {
...
<head>
  ...
  <title>Buyer Profile - Agrilink</title>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .buyer-info h3 { color: var(--primary); margin-bottom: 5px; font-size: 1.3rem; }
    .buyer-role { color: var(--text-muted); font-size: 0.9rem; margin-bottom: 20px; }
...
<?php include 'glow_bg.php'; ?>
  ...
  <nav class="navbar-standard">
    <div class="nav-inner">
      ...
      <nav class="navbar-nav-std">
        <a href="buyer_dashboard.php"><i class="fas fa-store"></i> Shop</a>
        <a href="buyer_profile.php" style="color:var(--secondary);"><i class="fas fa-user-circle"></i> Profile</a>
  ...
  <div class="main-content">
    ...
    <div class="page-header">
      <h2><i class="fas fa-id-card"></i> Buyer Profile & Settings</h2>
    </div>
    ...
    <div class="profile-grid">
      ...
      <div class="profile-sidebar">
        <div class="profile-picture-container">
          <?php if (!empty($buyer_info['profile_picture']) && file_exists($buyer_info['profile_picture'])): ?>
            <img src="<?= htmlspecialchars($buyer_info['profile_picture']) ?>" alt="Profile Picture" class="profile-picture">
          <?php else: ?>
          ...
          <form method="POST" enctype="multipart/form-data" style="display: inline;">
            ...
            <input type="hidden" name="full_name" value="<?= htmlspecialchars($buyer_info['full_name']) ?>">
            <input type="hidden" name="email" value="<?= htmlspecialchars($buyer_info['email']) ?>">
            <input type="hidden" name="phone" value="<?= htmlspecialchars($buyer_info['phone'] ?? '') ?>">
        ...
        <div class="buyer-info">
          <h3><?= htmlspecialchars($buyer_info['full_name']) ?></h3>
          <div class="buyer-role">External Buyer</div>
        </div>
          ...
          <form method="POST" enctype="multipart/form-data">
            <div class="form-grid">
              <div class="form-group">
                ...
                <input type="text" id="full_name" name="full_name" value="<?= htmlspecialchars($buyer_info['full_name']) ?>" required>
              </div>

              <div class="form-group">
                <label for="email"><i class="fas fa-envelope"></i> Email Address *</label>
                <input type="email" id="email" name="email" value="<?= htmlspecialchars($buyer_info['email']) ?>" required>
              </div>

              <div class="form-group">
                <label for="phone"><i class="fas fa-phone"></i> Phone Number</label>
                <input type="tel" id="phone" name="phone" value="<?= htmlspecialchars($buyer_info['phone'] ?? '') ?>" placeholder="+265 XXX XXX XXX">
              </div>
